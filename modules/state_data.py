...
class HiddenStateDataset(Dataset):
    def __init__(self, model, tokenizer, model_name, layer_name, target_idx, dataset_csv, window_size, vocab_size, device):
        ...
        # int: -1 is embedding, 0-31 for layers, 32 for logits right at the end
        ...
        for idx, doc in baukit.pbar(self.dataset_csv.iterrows()):
            self.doc_lengths[idx] = len(self.tokenize(
                str(doc['decoded_prefix']))) - 1  # subtract BOS
    ...
    def get_model_trace(self, text, idx):
        ...
        layers = [n for n, _ in self.model.named_modules() if re.match(
            r'^transformer.wte|^transformer.h.\d+$|^transformer.ln_f', n)]
    ...
    def tokenidx_to_docidx(self, token_idx):
        ...
        for doc_idx, bound in enumerate(cum):
            if token_idx < bound:
                relative_idx = token_idx - \
                    cum[doc_idx-1] if doc_idx > 0 else token_idx
    ...
    def __getitem__(self, index):
        ...
        inp, trout, layer_names = self.get_model_trace(
            str(doc['decoded_prefix']), doc_idx)
        ...
        target_token = torch.tensor(
            onehots[relative_idx + self.target_idx], device='cpu')
