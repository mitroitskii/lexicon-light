...
VOCAB_SIZE = llama.tokenizer.n_words

torch.set_default_dtype(torch.float32)

# ??? why use logilens?
def logitlens(state):
    return llama.model.output(state)
...
def train_epoch(epoch, probe, train_loader, criterion, optimizer, warmup_scheduler, accumulate, clip_threshold, batches_seen):
    ...
    for batch_idx, (hidden_states, targets, currs, _) in enumerate(train_loader):
        hidden_states, targets = hidden_states.to(device), targets.to(device)
        ...
        # concatenate the hidden state with the original embedding for the CURRENT token. 
        # don't need to add rotary position because that's already in the hidden_state. 
        with torch.no_grad():
            curr_embeddings = llama.model.tok_embeddings(_topktoks(currs).to(device)).squeeze()
            if len(curr_embeddings.shape) == 1: # undo too much squeezing
                curr_embeddings = curr_embeddings.unsqueeze(0)
            inputs = torch.cat([hidden_states, curr_embeddings], dim=1) # (bsz, 4096) + (bsz, 4096) = (bsz, 8192)
        ...
        if batch_idx % max(accumulate, 10) == 0 or batch_idx == len(train_loader) - 1:
            ...
            if torch.isinf(train_acc).any(): 
                ...
                print(_topktoks(output), _topktoks(targets))
...
def test(probe, test_loader, criterion, return_results=False, k=5):
    ...
    with torch.no_grad():
        for (data, targets, currs, doc_idxs) in baukit.pbar(test_loader):
            ...
            if len(curr_embeddings.shape) == 1: # undo too much squeezing
                curr_embeddings = curr_embeddings.unsqueeze(0)
            inputs = torch.cat([data, curr_embeddings], dim=1)
            ...
            for i, v in enumerate(output):         
                ...
                if return_results:
                    ...
                    curr_result = {
                        ...
                        "current_tok" : llama.tokenizer.decode(current_tok.tolist()),
                        "actual_tok" : llama.tokenizer.decode(actual_tok.tolist()),
                        "predicted_tok" : llama.tokenizer.decode(predicted_tok.tolist()),
                        "sourcehs_logitlens_tok_id" : sourcehs_logitlens.item(),
                        "sourcehs_logitlens_tok" : llama.tokenizer.decode(sourcehs_logitlens.tolist()),
                        **_topkprobs(v, llama.tokenizer)
                    }
...
def main(args):
    ...
    train_dataset = dset(llama.model, llama.tokenizer, MODEL_NAME, 
                        args.layer, args.target_idx, train_data, WINDOW_SIZE, VOCAB_SIZE, device)
    val_dataset = dset(llama.model, llama.tokenizer, MODEL_NAME,
                        args.layer, args.target_idx, val_data, WINDOW_SIZE, VOCAB_SIZE, device)
    test_dataset = dset(llama.model, llama.tokenizer, MODEL_NAME, 
                        args.layer, args.target_idx, test_data, WINDOW_SIZE, VOCAB_SIZE, device)
    
    # initialize matrix with inverted embed matrix if desired. have to stack it along with zeros 
    init_matrix = None 
    if args.probe_init == 'pinv':
        embed_weights = llama.model.tok_embeddings.weight # model.tok_embeddings.weight if "llama" in MODEL_NAME else
    ...
    # create linear probe, built to take in a hidden state and the -1 embedding of that hidden state!
    linear_probe = LinearModel(llama.model.params.dim * 2, llama.tokenizer.n_words, init_matrix=init_matrix).to(device)
    ...
    train_loader = DataLoader(dataset=train_dataset, batch_size=args.probe_bsz, collate_fn=collate_fn, 
        drop_last=True, pin_memory=True, shuffle=args.shuffle, generator=torch.Generator('cuda'), **kwargs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data', type=str, default='../data/train_100000.csv')
    parser.add_argument('--val_data', type=str, default='../data/val_10000.csv')
    parser.add_argument('--test_data', type=str, default='../data/test_10000.csv')

    # required specifications for where probe is trained 
    parser.add_argument('--layer', type=int, required=True,
                        help='which layer to train the probe at. from -1..32 where -1 is embedding layer and 32 is output.')
